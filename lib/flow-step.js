...
module.exports = function (option){
    ...
    function startStep(list, index) {
        function start() {
            if(index < list.length) {
                ...
                if(item.wait) {
                    if(item.needles) item.fun(item.needles, processCallback)
                    else item.fun(processCallback)
                        
                    function processCallback(err, json, processJson)  {
                        if(processJson) processData[curKey] = processJson
                        if(err && endHandler) endHandler(err, json) 
                        else startStep(list, index)
                    }
                }else{
                    if(item.needles) item.fun(item.needles)
                    else item.fun()
    ...
    this.next = function(step, ...needles) {
        ...
        if(option[step]) {
            params.fun = option[step].bind(that)
            if(needles.length) params.needles = needles
        }else{
            throw new Error(`function is not find: ${step}`)
        }
